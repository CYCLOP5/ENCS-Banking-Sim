...
def build_node_features(W: np.ndarray, df: pd.DataFrame) -> np.ndarray:
    ...
    lev = df['leverage_ratio'].values.astype(np.float64) if 'leverage_ratio' in df.columns else np.zeros(n)
    ib_ratio = df['interbank_ratio'].values.astype(np.float64) if 'interbank_ratio' in df.columns else np.full(n, 0.2)
    eq = df['equity_capital'].values.astype(np.float64) if 'equity_capital' in df.columns else np.maximum(ta - df['total_liabilities'].values, 0)
    deriv = df['deriv_ir_notional'].fillna(0).values.astype(np.float64) if 'deriv_ir_notional' in df.columns else np.zeros(n)
...
class SystRiskGCN(nn.Module):
    ...
    def __init__(self, in_channels: int, hidden1: int = 64,
                 hidden2: int = 32, hidden3: int = 16, dropout: float = 0.3):
        super().__init__()
...
def train_model(dataset: list, epochs: int = 80, lr: float = 1e-3,
                weight_decay: float = 1e-4, batch_size: int = 4,
                device: str = 'auto') -> SystRiskGCN:
    """
...
def load_trained_model(model_path: str = None, in_channels: int = 7,
                       device: str = 'cpu') -> SystRiskGCN:
...
def predict_risk(model: SystRiskGCN, W: np.ndarray, df: pd.DataFrame,
                 device: str = 'cpu') -> dict:
    """
...
def main():
    ...
    parser.add_argument('--runs', type=int, default=500, help='Number of MC runs for data gen')
    ...
    if args.train_only:
        if not DATASET_PATH.exists():
            print(f"ERROR: No dataset found at {DATASET_PATH}. Run with --generate-only first.")
            return
        dataset = torch.load(str(DATASET_PATH), weights_only=False)
        print(f"  Loaded dataset: {len(dataset)} graphs from {DATASET_PATH}")

    model = train_model(dataset, epochs=args.epochs, batch_size=args.batch_size, lr=args.lr)
