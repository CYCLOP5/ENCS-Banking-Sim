...
def status_to_label(status_array: np.ndarray) -> np.ndarray:
    ...
    # Normalise to lowercase + strip whitespace for robustness
    normalised = np.array(
        [
    ...
    # Sanity check — if the simulation produced no risky nodes at all, surface it
...
def _single_mc_run(args):
    ...
    try:
        ...
        if run_idx < 5:  # log first few failures for debugging
            traceback.print_exc()
        else:
            print(
    ...
    try:
        ...
        # This single run produced all-safe nodes — skip it
        return None
...
def train_model(
    ...
    # ── Stratified train / val split ──────────────────────────────────
    # Use per-graph majority label as the stratification key.
    ...
    try:
        ...
        # Fallback if stratification is impossible (only one class)
        train_idx, val_idx = train_test_split(
    ...
    # ── Balanced class weights via sklearn ─────────────────────────────
    ...
    for epoch in range(1, epochs + 1):
        ...
        # ── Validation with richer metrics ─────────────────────────────
        model.eval()
        val_preds_list: list = []
    ...
    if MLFLOW_AVAILABLE:
        ...
        # Log the PyTorch model to the MLflow model registry
...
def main():
    ...
    # Allow disabling MLflow at runtime
